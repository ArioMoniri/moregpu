...
| `POST /vision/infer` | `{id, shape, data, worker?}` | Routed to any holder. Torch: `vision_infer` over the train relay. WebGPU: `vision_infer` over the sealed model RPC. Both replies are normalised to `{shape, data, kind, worker}`. For a segment/classify model the reply also has `pred_sha256`, `pred_shape` and `pred_dtype` ([VISION.md](VISION.md#prediction-hashes-pred_sha256)). A WebGPU holder only gets inputs of the lowered static shape. |
| `POST /vision/infer_batch` | `{id, inputs:[{shape, data}], workers?, check_parity?, reference?}` | The inputs are spread over **all** holders by the work-stealing `VisionBatch` queue. A per-worker-kind `task` callback builds each kind's payload. Returns `outputs` in input order (each with `worker`/`kind` and, for a segment/classify model, `pred_sha256`), `per_worker` counts, `failed`, and with `check_parity` the `parity: {reference, max_abs, per_worker}` against a torch reference worker. |
| `POST /vision/batch` | as before | Volume batches (`vision_predict`: sliding window, TTA, label maps) stay on the torch holders. Each result in the job record (`GET /vision/jobs/:id?results=1`) carries the written label map's `pred_sha256` and `pred_dtype`. |
