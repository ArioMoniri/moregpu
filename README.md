...
- **Still not built:** a **tensor-core** GEMM (the portable WGSL path is fp32/fp16 **plus int8/int4 dequantize-in-shader** — a memory/bandwidth win, not a compute-speed one; WebGPU has no tensor cores, and bitsandbytes int8/nf4 still runs only on the CUDA torch worker), **coordinator-side** quantize-and-stream and auto-streaming a first stage's `tokenizer.json` (the WebGPU worker now *ingests* both — a quantized per-stage checkpoint and an on-device tokenizer — but the server does not yet *produce* them automatically; both were driven through the push-staging path in the verification), fleet-scale observability, and kernel-enforced resource limits.
