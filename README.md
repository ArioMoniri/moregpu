...
> [!NOTE]
> **Can it run LLMs?** Yes — a *small* one. `examples/llm_infer.py` runs a real **GPT-2** on the pool and matches Hugging Face token-for-token. It's **slow** (fp32, and every layer's activations round-trip over the network — seconds/token), so big/low-latency models stay impractical. MoreGPU's backend is **WebGPU (WGSL)**: portable across NVIDIA/AMD/Apple/Intel, but fp32 compute-only — no tensor cores, no fp16/int8, no CUDA/PTX, no autograd. Real serving speed and big models would need a **separate native fp16/CUDA worker type** (roadmap, not built). What ships is a correct, verified fp32 primitive set you can compose into real models at small scale — it won't masquerade as a fast CUDA serving stack it isn't.
