...
- **Still not built:** a hand-written int8/**tensor-core** GEMM (the portable WGSL path stays fp32/fp16; bitsandbytes int8/nf4 runs only on the CUDA torch worker), an in-browser tokenizer for a browser *first* stage (the client tokenizes today), fleet-scale observability, and kernel-enforced resource limits.
