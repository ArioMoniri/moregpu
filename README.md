...
| Big-model inference across machines (pipeline sharding) | 🟡 | **Pipeline-parallel sharding** ([`llm_shard.py`](examples/llm_shard.py)): split a model's transformer layers into contiguous stages, one per torch worker, piping only **activations** (`~[seq×hidden]`, never weights) — the low-bandwidth path (Petals / Mesh-LLM style). Works for **GPT-2 *and* Llama-family** (RMSNorm/RoPE) — verified token-for-token **exact match** for GPT-2 (6+6 split) *and* SmolLM-135M (15+15 split) across 2 workers, each worker holding **only its stage**. **Download-free** too (`POST /model/shard {push:true}`): the coordinator fetches the model once and streams each worker only its stage's tensors (its layer slice + embeddings/head on the ends), rebuilt as a per-stage safetensors — so the fleet never touches the hub; verified byte-for-byte identical output to a full forward. *Honest note:* sharding is for **capacity** (a model too big for one node), **not speed** — a single stream flows stage→stage sequentially, so over a WAN it's **slower per token** than one node, and per-token forwards can 502 on a flaky free tunnel. Still no int8/tensor cores. **fp16 sharding** (`{push:true, fp16:true}`) halves each stage's VRAM on a GPU worker, so a model up to ~2× larger fits the same fleet (a 7B ≈ 14 GB fp16 instead of 28 GB); activations still pipe as fp32, and it's **verified argmax-identical to the fp32 shard on a real GPU** (tiny GPT-2/Llama on MPS, max\|Δlogit\|≈2e-4). Also: explicit per-stage **`{split}`** for heterogeneous fleets; **int8/nf4** and **pre-quantized `quant:'auto'`** (GPTQ/AWQ/bnb-4bit) stages on CUDA torch workers (a 7B fits an ~8 GB slice, verified live on an H200); and — new — **a WebGPU worker can hold a stage**, even a whole small model in a **browser tab**: a hand-written WGSL transformer stage (RMSNorm/GQA-RoPE/causal/SwiGLU, embed-gather, LM head, KV-cached decode) verified **token-for-token vs PyTorch** (logits ~1e-6), run live as `torch → browser-tab → torch`; that WebGPU stage now also holds **int8/int4** weights (dequantize-in-shader, so a browser/mobile tab fits a bigger slice) and can **tokenize text on-device** on a *first* stage (byte-level BPE) — a browser tab as a self-contained, quantized stage. The whole flow is **one CLI command** — `moregpu shard MODEL [--quant wq8\|wq4] [--actq int8] [--prompt "…"]` — where the coordinator **quantizes each stage to int8/int4 and streams it download-free** to a WebGPU worker (verified argmax == PyTorch for wq8/wq4), the first stage **tokenizes on-device**, and `--actq int8` shrinks the recurring per-token inter-stage wire ~4× (fp32 kept into the head). |
| Training / fine-tuning (LoRA) | 🟡 | **LoRA fine-tuning on the torch worker** — single-worker ([`lora_finetune.py`](examples/lora_finetune.py)) *and* **distributed across N workers via DiLoCo** ([`lora_distributed.py`](examples/lora_distributed.py)): each worker trains its own shard for H local steps, the **coordinator averages the adapters + applies an outer Nesterov step** (a real reduce path), broadcasts the global. Single-worker is verified **bit-for-bit** against a seeded reference; distributed DiLoCo matches a seeded reference to **fp tolerance** (losses ~2e-5, adapter ~1e-3) and was run **live across a heterogeneous fleet** — a Colab **T4 (CUDA)** + this Mac **(MPS)** training one adapter over a public tunnel (loss 4.45 → 1.64 / 3 rounds). Synchronous DiLoCo; async + secure-aggregation are the next step. Needs the torch worker, not the WGSL path. **Download-free fine-tuning** (`moregpu finetune MODEL --push`, or `POST /train/load {push:true}`): the coordinator streams the base to the worker, which **never touches the HF hub** — so a **no-download node** (a laptop worker on a tunnel, a locked-down desktop) can fine-tune a model it never fetched. Verified end-to-end against a mock hub (base streamed → LoRA learns → generate), no regression to the normal download path. |
...
| Conv2d / CNNs / 3D UNets (vision inference) | ✅ | **Native torch workers** run any published vision model as released: torchvision, timm, MONAI, HF, allowlisted plugins, torch.export, TorchScript or ONNX, all sha256-verified; pickles are refused ([MODELS.md](docs/MODELS.md)). **WebGPU / browser workers** run models **automatically lowered** to a WGSL op-graph: conv2d/3d implicit GEMM, transposed conv, norms, pooling, upsampling, attention, fp16 storage. Parity vs PyTorch is ≈1e-6, verified on real software adapters (lavapipe, SwiftShader). Whole-volume sliding window + flip TTA is equivalent to MONAI; see [WEBGPU_VISION.md](docs/WEBGPU_VISION.md). |
| Distributed vision inference | ✅ | `/vision/batch`: a **work-stealing case queue** over the fleet with churn retry and straggler duplication, plus **tile sharding** (`split: "tiles"`), which splits one large volume across workers and returns exactly the single-node result. `/vision/infer_batch` runs on a **mixed fleet** of torch + WebGPU + browser workers with parity checks. |
| JEPA self-supervised pretraining | ✅ | `ijepa_2d`, `jepa_2p5d` and `jepa_3d` ([JEPA.md](docs/JEPA.md)). A ViT with timm-compatible keys and I-JEPA multi-block masking, trained with **DiLoCo across N torch workers**. The EMA target is updated identically on every worker from coordinator-supplied progress. Collapse monitors: RankMe and per-dimension std. Exports to safetensors, torch.export or ONNX; the result equals the in-process reference to 1e-6, and resume is exact. |
| Fine-tuning any vision model | ✅ | `segment` (2D / 2.5D / 3D, Dice+CE), `classify`, and **`finetune_model`**, which fine-tunes **any published native model** (torchvision / timm / MONAI / HF / plugin) in full, head-only or LoRA mode with DiLoCo. BatchNorm statistics are averaged correctly ([TRAINING.md](docs/TRAINING.md)). |
| Generic training sessions | ✅ | `/train/sessions` runs any `TrainTask` over N workers. Features: weighted DiLoCo; deterministic seeded shards; heterogeneous allocation; exact `target_samples`; warmup + cosine LR; bf16/fp16/int8-delta sync; checkpoint and bit-exact resume including per-worker optimizer state; churn handling; per-round telemetry (compute / data / serialise / network / wait, bytes, GPU util, energy). Available from the CLI (`moregpu train …`), both SDKs and the dashboard. |
| Custom CUDA/PTX kernels · int8 GEMM · Stable Diffusion · OptiX · NVENC | ❌ | The portable WGSL path is fp32/fp16 plus an **int8/int4 dequantize-in-shader** GEMM for shard weights (a memory/bandwidth win, not a tensor-core speed-up) — none of these CUDA/PTX/graphics/codec paths. The opt-in **torch worker does run on CUDA via PyTorch** on NVIDIA — **verified live**: GPT-2 served on a Colab **T4 (CUDA)** through the pool at ~44 tok/s, token-for-token exact match. PyTorch/cuBLAS uses **tensor cores automatically for large fp16/bf16 GEMMs**; small-model single-stream decode is memory-bound and sees no tensor-core benefit (measured fp16 ≈ fp32 on the T4). MoreGPU writes **no custom CUDA/PTX kernels** and no graphics/codec paths; its portable WGSL path now hand-writes an **int8/int4 dequantize-in-shader GEMM** (a memory/bandwidth win so a browser/mobile tab holds a bigger slice — WebGPU has no tensor cores, so no compute-speed gain), and the torch worker loads **int8/nf4 (bitsandbytes)** and **pre-quantized GPTQ/AWQ/bnb-4bit** weights on CUDA for shard stages, so a 7B fits an ~8 GB slice. |
...
### 🩻 Vision & training at a glance (0.7)
...
**What each worker type can do** — each capability's test evidence is listed in [docs/VISION.md](docs/VISION.md#support-matrix):
...
The swarm splits vision work **by data**, not by layers:
- **Training** uses DiLoCo: every worker holds the whole model and trains on its own shard, and the workers sync once per
  round.
- **Inference** uses the case queue or tile sharding.

Layer (pipeline) sharding, as used below for LLMs, is only for models too big for one device.

A case study (pooling small GPUs vs one large GPU for JEPA pretraining on liver CT) will be summarised in
`docs/case-studies/` once the study is complete.
...
> [!TIP]
> **Running across machines, in practice.** On a **LAN** (the target: idle institutional desktops), weight transfer is instant, so download-free sharding/serving/fine-tuning across many nodes just works — start each node with the worker one-liner (see the **Worker track** below) pointed at the coordinator. Over a **public tunnel** (e.g. a laptop joining a cloud coordinator), the *one-time weight transfer* to that node is bandwidth-bound — a real-model shard stage or a fine-tune base can be minutes-to-hours on a free ~1 Mbps tunnel (capacity, not speed); keep the remote node's share small, or use a LAN. A node that **must not download models** (locked-down, or you just don't want model files on it) still joins fully: **`moregpu finetune MODEL --push`** and **`POST /model/load {push:true}`** stream the weights *from the coordinator*, so that node never touches the HF hub. And a free **Colab** coordinator idles out after inactivity — keep it alive by running a cell that stays busy (`import time; exec("while 1: time.sleep(45)")`) alongside the pool.

**Toward hosting Kimi-class models on idle desktops** (the honest roadmap): dense-model **download-free pipeline sharding** works today (`POST /model/shard {push:true}`, verified byte-for-byte), now **fp16** too. **Expert parallelism** for a **pure-routed MoE** (OLMoE / Qwen3-MoE) is also **shipped and exact-match-verified** (`POST /model/moe_shard`): the router lives on a backbone node, the experts are split across holder nodes, each token routes to only the ~k holding its active experts, and the coordinator (or an opt-in peer mesh) combines the partials — token-argmax identical to the un-sharded MoE. **fp16** (`{fp16:true}`, verified argmax-identical on a real GPU, max\|Δlogit\|≈2e-4) halves the per-holder VRAM, so a real **OLMoE-1B-7B (~14 GB fp16, was ~28 GB)** now fits within a small fleet's reach. What a ~1T-param **Kimi K2** still needs is the harder part — **shared-expert / group-router** MoE families (only pure-routed is supported today), **int4** quant (even ~32B active is a lot), and surviving node churn — a **months-long build, not shipped**; that's the remaining direction, not a claim.
...
> [!TIP]
> 🌐 **Contribute from a browser tab — zero install.** Open the worker page, paste the coordinator URL + join token, and the tab's GPU joins the pool over WebGPU. It runs kernels **and holds a real model shard** — a pipeline stage, or a whole small model — using the *same* [`apps/worker/worker.ts`](apps/worker/worker.ts) that runs under Deno (a platform shim runs it in either), driving a hand-written WGSL transformer stage (RMSNorm + GQA-attention + RoPE + causal mask + SwiGLU, embedding-gather, LM head, and KV-cached decode) that is verified **token-for-token against PyTorch** (logits within ~1e-6) — now also running **int8/int4** weights (dequantize-in-shader, a bigger slice per tab) and **tokenizing text on-device** for a first stage (byte-level BPE). Build + serve it:
> ```sh
> deno bundle --platform browser apps/worker/worker.ts -o apps/worker/worker.bundle.js
> # serve apps/worker/ over http(s), then open  index.html?server=wss://…/ws&token=<join-token>
> ```
> The coordinator negotiates **capabilities** (`caps`), so a tab is placed only where it fits. A browser tab is best as a **throughput / parallel** donor — single-user *interactive* decode is latency-bound by round-trips over the internet, and mobile GPUs cap buffer sizes — so a fleet of tabs shines at batched serving and embarrassingly-parallel work rather than one low-latency chat stream.
...
coordinator's `/whoami`), falling back to relay. Most recently, a **GPU-residency pass** on the WebGPU shard
runtime — **resident weights** (uploaded to VRAM once) + a **resident KV cache** (persistent buffer, appended in
place; 513× fewer upload bytes at token 512) + an **opt-in fused decode** (`MOREGPU_FUSED_DECODE`, a whole cached
stage in 1 submit + 1 map; bit-identical across fp32/int8/int4) — landed alongside a dual-lens code audit whose
top findings became six merged fix PRs (mandatory result signatures, memory-bounded quant streaming, resource
caps). Release-signature freshness is now a **release-tag CI gate** rather than a per-push one, so ongoing
`worker.ts` work keeps CI green while a tagged release still fails closed until re-signed.

**Still not built:** async DiLoCo (no straggler barrier), cross-tenant secure aggregation, **tensor-core / int8 compute speedups** (the WGSL int8/int4 path is a memory win only),
...
- **WebGPU workers hold model shards + capability negotiation** — a **browser tab** (or any WebGPU device) now hosts a pipeline stage, or a *whole* small model, via a hand-written WGSL transformer runtime (RMSNorm · GQA-attention + RoPE + causal mask · SwiGLU · embedding-gather · LM head · **KV-cached, any-length decode**), verified **token-for-token against PyTorch** (~1e-6) and run live `torch → browser-tab → torch`. The coordinator negotiates each worker's `caps` and places stages accordingly (torch stays for autograd / whole-model residency).
- **Quantized + heterogeneous sharding** — **int8/nf4 (bitsandbytes)** and **pre-quantized `quant:'auto'`** (GPTQ/AWQ/bnb-4bit) shard stages on CUDA torch workers (a **7B verified live** across a T4 + an H200 slice), plus explicit per-stage **`{split}`** so a heterogeneous fleet places layers where the VRAM/bandwidth is.
- **A WebGPU worker holds *quantized* shards + tokenizes on-device** — the portable WGSL runtime now runs **int8 / int4** decoder-block linears (dequantize-in-shader, needing no packed-dot / 8-bit-storage feature → portable across Metal/Vulkan/D3D12), so a browser/mobile tab holds a slice at **~1 B/weight** (int8, 4× vs f32 / 2× vs f16) or **~0.5 B/weight** (int4) and a push-streamed stage is smaller. Verified whole-model **argmax-identical to PyTorch** on a tiny Qwen2 (int8, quant loss ~3e-3), the int4 path matching a torch int4-dequant reference to ~1e-7. And a **first** stage can turn **text → token ids on-device** (byte-level BPE; gpt2 + qwen2/gpt4 pre-tokenizer variants; loads a HF `tokenizer.json` or vocab+merges), so a browser tab is a **self-contained first stage** — encode now **exact-matches the real Qwen2.5 tokenizer** (70 diverse cases incl. NFC-normalized decomposed unicode; and gpt2, 67 cases). This is now driven **end-to-end from the CLI**: `moregpu shard MODEL --quant wq8` has the **coordinator quantize each stage** to int8/int4 + scale and stream it **download-free** to a WebGPU worker (verified live coordinator→worker: whole-model argmax == PyTorch for **wq8 AND wq4**), and `moregpu shard --prompt …` runs text-in/out with the WebGPU first stage tokenizing on-device (`shard_tok`/`shard_detok`). *Honest scope:* WebGPU still has no tensor cores — this is a memory/bandwidth win, not compute speed.
- **int8 activations on the wire (opt-in)** — `moregpu shard --actq int8` encodes the per-token hidden state between **middle** stages as symmetric per-row int8 (~4× less per-hop wire; fp32 kept into the head boundary — logits are the most argmax-sensitive and quant error compounds per hop). Verified live on a 3-worker split: argmax stayed == PyTorch while the int8 boundary measurably perturbed the logits (maxErr 1.9e-3 — proving it fired, not a silent fp32 fallback); the `worker.ts` and `worker_torch.py` encoders are **byte-identical**, so a torch stage and a WebGPU stage interoperate across a boundary. Lossy and compounds per hop (per-row scale is outlier-sensitive) → opt-in, and never applied into the head.
...
- **WebGPU GEMM speed — vec4 loads (bit-identical):** the shard fp32 GEMM now reads x/w as `vec4<f32>` — **vec4x4** (4 output cols/thread) for prefill, plain **vec4** for a 1-row decode — which is **bit-for-bit identical** to the naive kernel (measured maxErr 0.0) and **~1.6–2.3× faster** (needs K%4==0, true for every Qwen/Llama linear; naive fallback otherwise). This is the lever that actually works on Apple Silicon: shared-memory **tiling** measured *slower* here (0.87× prefill, 0.25× decode — the unified cache already captures the reuse tiling exists for), and `dot4I8Packed` runs only as a software lowering (slower than fp32 vec4, and lossier). WebGPU still has **no tensor cores** and no hardware int8-compute path — that would need a backend exposing DP4A.
- **Resident weights + KV, and an opt-in fused decode — the WGSL decode path stops re-uploading every token.** Three GPU-residency fixes closed the shard runtime's biggest inefficiency. (1) A stage's **weights upload to VRAM once** (identity-keyed cache) instead of being re-streamed host→GPU on every matmul. (2) Each layer's **KV cache is now a persistent GPU buffer grown by doubling** — a decode step appends only the new token rows at the tail (GPU→GPU) instead of reallocating and re-uploading the whole growing cache every step: **O(n²)→O(n)** host traffic, **513× fewer upload bytes at token 512** for a 3B GQA shard. (3) An **opt-in fused decode path** (`MOREGPU_FUSED_DECODE=1`) keeps activations resident across a whole cached stage — every op is one compute pass on a single command encoder, mapped to CPU **once** — collapsing a decode stage from ~15·layers GPU↔CPU round-trips to **1 submit + 1 map** (**≈68× faster/token** on the Deno worker for a 0.5B, where each `mapAsync` is a real round-trip; the portable headline is the **360×** structural reduction at 24 layers). All three are **bit-identical**: weights/KV verified byte-for-byte, and the fused path matches both the unfused runtime *and* an independent uncached golden **token-for-token across fp32 / int8 / int4**, crossing the KV-cache resize. Still no tensor cores — this cuts host round-trips, not per-op compute.
- **Still not built:** a tensor-core / hardware-int8 compute path (see above), fleet-scale observability, and kernel-enforced resource limits. bitsandbytes int8/nf4 still runs only on the CUDA torch worker.
