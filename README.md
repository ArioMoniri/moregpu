...
- **Still not built:** a **compute-speed** WebGPU GEMM. WebGPU has no tensor cores, and — measured on this Metal backend — a shared-memory-**tiled** GEMM ran *slower* than the naive one (0.5–0.67× at prefill, worse at decode), while the packed-int8-dot (`dot4I8Packed`) fast path compiles but its compute speedup is unverified — so the portable kernel stays the (fastest-here) **naive dequantize-in-shader** one, and int8/int4 remain a memory/bandwidth win, not a compute one (a real int8-compute win needs a backend that exposes packed-integer-dot). bitsandbytes int8/nf4 still runs only on the CUDA torch worker. Also: fleet-scale observability and kernel-enforced resource limits.
