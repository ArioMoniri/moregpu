...
- **Python torch worker**: with `--pin`/`MOREGPU_PIN` set it completes the handshake with CA verification disabled, reads the leaf cert off the live socket, and refuses to join unless `sha256(DER) == MOREGPU_PIN` (the self-signed default). With **no pin** it keeps **normal CA verification**, so a real-CA coordinator (a tunnel like trycloudflare, or Let's Encrypt behind a reverse proxy) authenticates and connects, while a **self-signed** coordinator **fails the handshake** — the join token is never sent to an unauthenticated endpoint. (`MOREGPU_ALLOW_UNPINNED=1` forces verification off for local hacking only.)
