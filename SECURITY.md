...
*Honest limit:* the opt-in worker↔worker **peer transport** is not itself wrapped in TLS — but its activation frames are already AEAD-sealed (AES-256-GCM) and Ed25519-signed with the pool key at the application layer, so a peer-mesh eavesdropper sees only ciphertext. Wrapping the peer hop in TLS too is future work.
