#!/usr/bin/env -S deno run --unstable-webgpu --allow-net --allow-env
/**
 ...
 *   deno run --unstable-webgpu --allow-net --allow-env <this-url> \
...
// Duty cycle: fraction of time allowed to compute (rest is sleep). Lower = gentler + less power.
let DUTY = args.has('throttle') ? Math.max(0.05, Math.min(1, Number(args.get('throttle')))) : NaN;
  ...
  return { kind: 'cpu', label: `cpu:${Deno.build.arch}`,
    ...
    async matmul(a, b, M, N, K) {
      ...
      for (let i0 = 0; i0 < M; i0 += chunk) {
        ...
        if (DUTY < 1) await sleep(busy * (1 / DUTY - 1)); // duty 0.6 → sleep ≈ 0.67×busy
      }
      return o;
    } };
}
...
function connect() {
  ...
  ws.onopen = () => { ws.send(JSON.stringify({ t: 'register', joinToken: TOKEN, node: { id: NAME, backend: backend.kind, label: backend.label, os: Deno.build.os } })); };
  ws.onmessage = async (ev) => {
    ...
    if (msg.t === 'welcome') { tenantKey = b64d(msg.tenantKeyB64); if (Number.isNaN(DUTY)) DUTY = typeof msg.duty === 'number' ? msg.duty : 0.6; console.log(`[worker] joined pool · duty-cycle=${(DUTY * 100).toFixed(0)}% (compute/idle split)`); return; }
    if (msg.t === 'assign') {
      ...
      try {
        ...
        const out = req.kernel === 'matmul' ? await backend.matmul(b64ToF32(req.a), b64ToF32(req.b), req.rows, req.N, req.K) : await backend.vectorAdd(b64ToF32(req.a), b64ToF32(req.b));
        ...
        if (DUTY < 1) await sleep(ms * (1 / DUTY - 1)); // cool-down between shards → gentler + less power
      } catch (e) { ws.send(JSON.stringify({ t: 'result', shardId: msg.shardId, jobId: msg.jobId, ok: false, error: String(e) })); }
    }
  };
  ws.onclose = () => { console.log('[worker] disconnected, retrying in 2s'); setTimeout(connect, 2000); };
